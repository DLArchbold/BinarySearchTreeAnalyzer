...
struct BST {
		...
		bool deepestNodes();
		bool PrintDeepestNodes(BTNode*, int);
		int Height(BTNode *);
		//int MaxHeight(int, int, string &);
		bool display(int , int );
		void inOrderPrintToFile();
		void inOrderPrint2ToFile(BTNode *, ofstream &);
		void ReverseinOrderPrint();
		void ReverseinOrderPrint2(BTNode *);
		void ReverseinOrderPrintToFile();
		void ReverseinOrderPrint2ToFile(BTNode *, ofstream &);
			
		bool CloneSubtree(BST, type);
		bool FindBTNClone(BST &, BTNode *, type);
		void InsertSubNodes(BST &, BTNode *);
		...
		bool FindBTNode(BTNode *, type, int &);
		...
		bool printSpiral();
		void printSubLevels(BTNode *, int, int);
};
